---
title: "Analysis of Fox Squirrel data"
...
PCC <- c(cc_min_max, cc_min_hr, cc_max_hr, cc_hr_dur, cc_hr_min, cc_hr_dist, cc_hr_a, cc_min_dur, cc_min_dist, cc_min_a, cc_tree_hr, cc_tree_dur, cc_tree_dist, cc_tree_a, cc_tree_oak, cc_tree_prop, cc_oak_hr, cc_oak_dur, cc_oak_dist, cc_oak_a, cc_oak_prop, cc_dist_a, cc_dist_dur, cc_a_dur, cc_prop_dist, cc_prop_a)
parameter.1 <-c("min T", "min T", "max T", "hour", "hour", "hour", "hour", "estimated min T", "estimated min T", "estimated min T", "tree dens", "tree dens", "tree dens", "tree dens", "tree dens", "tree dens", "oak dens", "oak dens", "oak dens", "oak dens", "oak dens", "dist", "dist", "area", "prop of oaks", "prop of oaks")
parameter.2 <- c("max T", "hour", "hour", "dur", "estimated min T", "dist", "area", "dur", "dist", "area", "hour", "dur", "dist", "area", "oak dens", "prop of oaks", "hour", "dur", "dist", "area", "prop of oaks", "area", "dur", "dur", "dist", "area")
...
# Models incorporating minimum or maximum temperature have lower AICc than similar models using hour. However, using estimated minimumum temperature instead, similar models using hour have lower AICc. 
```
...
## Comparing the highest-ranked models against single-variable, null, and global models

```{r}
models_3 <- list(model_12a, model_16a, m_min.T.est, m_hour, m_dur.min, m_length, m_area, m_tree.dens, m_oak.dens, m_prop.oak, null, global)

mod_names_3 <- c('model_12a', 'model_16a', 'm_min.T.est', 'm_hour', 'm_dur.min', 'm_length', 'm_area', 'm_tree.dens', 'm_oak.dens', 'm_prop.oak', 'null', 'global')
...
# Linear regression (performed here for the two highest-ranked models by AICc and for the three additional models whose outputs are included in the supplemental tables)
ggplot(model_16a, aes(sample=model_16a$residuals)) +
  stat_qq() + 
  stat_qq_line() +
  ylim(c(-500, 500)) +
  labs(title = "squirrel density = tree density*hour + distance", x = "Sample Quantiles", y = "Theoretical Quantiles")
...
ggplot(model_16b, aes(sample=model_16b$residuals)) +
  ...
  stat_qq_line() +
  ylim(c(-500, 500)) +
  labs(title = "squirrel density = oak density*hour + distance", x = "Sample Quantiles", y = "Theoretical Quantiles")

ggplot(model_17b, aes(sample=model_17b$residuals)) +
  stat_qq() + 
  stat_qq_line() +
  ylim(c(-500, 500)) +
  labs(title = "squirrel density = oak density*oak proportion + hour + distance", x = "Sample Quantiles", y = "Theoretical Quantiles")
...
# 5. Calculate Jacob's Selection Index of trees were occupied by a squirrel and that squirrels were nearest to.
```{r}
# Import data
